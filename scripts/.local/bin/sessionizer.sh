#!/usr/bin/env bash
CONFIG_FILE_NAME="sessionizer.conf"
...
# config file example
# ------------------------
# # file: ~/.config/sessionizer/sessionizer.conf
...
# TS_MAX_DEPTH=2
# This is not meant to override .sessionizer.  At first i thought this
...
#
# how to use:
# sessionizer -w 0 will execute the first command in window -t 69.
# TS_SESSION_COMMANDS=(<cmd1> <cmd2>)
#
# TS_LOG=true # will write logs to ~/.local/share/sessionizer/sessionizer.logs
# TS_LOG_FILE=<file> # will write logs to <file> Defaults to ~/.local/share/sessionizer/sessionizer.logs
# ------------------------
...
if [[ $TS_LOG != "true" ]]; then
    if [[ -z $TS_LOG_FILE ]]; then
        TS_LOG_FILE="$HOME/.local/share/sessionizer/sessionizer.logs"
    fi
...
log "sessionizer($VERSION): idx=$session_idx cmd=$session_cmd user_selected=$user_selected split_type=$split_type log=$TS_LOG log_file=$TS_LOG_FILE"
